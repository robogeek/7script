/* cgic.c - CGI functions for 7Script.  Originally developed by
 *	Thomas Boutell and modified for use here.
 */
#if CGICDEBUG
...
/* XXX Provide an idiom or easy way for the calling script 
 * to detect that a command fails and to recover gracefully.
 */

int Cgic_Commands(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {

     /*printf("Cgic_Commands: %s: %s (%d)\n", argv[0], argv[1], argc);*/
     if (argc <= 1) {
	  Tcl_AppendResult(interp, "cgi USAGE: cgi arg ...", NULL);
	  return TCL_ERROR;
     }
	 ...
	 * cgic args
	 *
	 * Get the names of all the args supplied in the form.
	 */
	...
	if (argc != 2) {
	     Tcl_AppendResult(interp, "USAGE: cgi args", NULL);
	     return TCL_ERROR;
	}

	Tcl_DStringInit(&ds);
	for (e = cgiFormEntryFirst; e; e = e->next) {
	    Tcl_DStringAppendElement(&ds, e->attr);
	}
	Tcl_DStringResult(interp, &ds);
	return TCL_OK;
    }
    else if (strcmp(argv[1], "exists") == 0) {
	if (cgiFormEntryFindFirst(argv[2]))
	...
	return TCL_OK;
         ...
         * cgic get name
         *
         * Get all the values supplied as form arguments by this name.
         * Usually just one value is supplied for each form argument,
         * and sometimes more than one is supplied.  If necessary this
         * is returned as a TCL list.
         */

      /* XXX better argument checking */

        cgiFormEntry *e;
        Tcl_DString ds;
        Tcl_DStringInit(&ds);
        for (e = cgiFormEntryFindFirst(argv[2]); e; e = cgiFormEntryFindNext())
{
            Tcl_DStringAppendElement(&ds, e->value);
        }
        Tcl_DStringResult(interp, &ds);
        return TCL_OK;
    }
        ...
        /*
	 * cgic int name [ min max ]
	 *
	 * Return the value for 'name' checking that it is an integer
	 * and optionally range checking it to be between min and max.
	 */
	...
	char *name = argv[2];
	if (argc == 3)
	  ret = cgiFormInteger(name, &result, 0);
	else if (argc == 5) {
	  int min, max;
	  if (Tcl_GetInt(interp, argv[3], &min) == TCL_ERROR) {
	    Tcl_AppendResult(interp, "cgic int ", name, ": min", NULL);
	    return TCL_ERROR;
	  }
	  if (Tcl_GetInt(interp, argv[4], &max) == TCL_ERROR) {
	    Tcl_AppendResult(interp, "cgic int ", name, ": max", NULL);
	    return TCL_ERROR;
	  }
	  ret = cgiFormIntegerBounded(name, &result, min, max, 0);
	}
	else
	  ret = cgiFormBadType;  /* XXX Better error */
	switch (ret) {
	case cgiFormSuccess:
	...
	case cgiFormEmpty:
	    Tcl_AppendResult(interp, "cgic int '", name,
			     "': empty value in form input", NULL);
	    ret = TCL_ERROR;
	    break;

	case cgiFormBadType:
	    Tcl_AppendResult(interp, "cgic int '", name,
			     "': non-integer value in form input", NULL);
	    ret = TCL_ERROR;
	    break;
	...
	return ret;
      ...
      for (c = token[0]; token[0] != '\0'; token++, c = token[0]) {
	if ((((int)c) < 32)
	 || (((int)c) >= 127)) {
	  sprintf(b, "%%%02X", (unsigned int)(c & 0xff));
	  Tcl_DStringAppend(&res, b, -1);
	  continue;
	}
	switch (c) {
	case ' ':  Tcl_DStringAppend(&res, "+", -1); break;
        case '!':  Tcl_DStringAppend(&res, "%21", -1); break;
        case '"':  Tcl_DStringAppend(&res, "%22", -1); break;
        case '#':  Tcl_DStringAppend(&res, "%23", -1); break;
        case '$':  Tcl_DStringAppend(&res, "%24", -1); break;
        case '%':  Tcl_DStringAppend(&res, "%25", -1); break;
        case '&':  Tcl_DStringAppend(&res, "%26", -1); break;
        case '\'': Tcl_DStringAppend(&res, "%27", -1); break;
        case '(':  Tcl_DStringAppend(&res, "%28", -1); break;
        case ')':  Tcl_DStringAppend(&res, "%29", -1); break;
        case '+':  Tcl_DStringAppend(&res, "%2B", -1); break;
        case ',':  Tcl_DStringAppend(&res, "%2C", -1); break;
        case '/':  Tcl_DStringAppend(&res, "%2F", -1); break;
        case ':':  Tcl_DStringAppend(&res, "%3A", -1); break;
        case ';':  Tcl_DStringAppend(&res, "%3B", -1); break;
        case '<':  Tcl_DStringAppend(&res, "%3C", -1); break;
        case '=':  Tcl_DStringAppend(&res, "%3D", -1); break;
        case '>':  Tcl_DStringAppend(&res, "%3E", -1); break;
        case '?':  Tcl_DStringAppend(&res, "%3F", -1); break;
        case '@':  Tcl_DStringAppend(&res, "%40", -1); break;
        case '[':  Tcl_DStringAppend(&res, "%5B", -1); break;
        case '\\': Tcl_DStringAppend(&res, "%5C", -1); break;
        case ']':  Tcl_DStringAppend(&res, "%5D", -1); break;
        case '^':  Tcl_DStringAppend(&res, "%5E", -1); break;
        case '`':  Tcl_DStringAppend(&res, "%60", -1); break;
        case '{':  Tcl_DStringAppend(&res, "%7B", -1); break;
        case '|':  Tcl_DStringAppend(&res, "%7C", -1); break;
        case '}':  Tcl_DStringAppend(&res, "%7D", -1); break;
        case '~':  Tcl_DStringAppend(&res, "%7E", -1); break;
	default:
	  b[0] = c;
	  b[1] = '\0';
	  Tcl_DStringAppend(&res, b, -1);
...
#else
int Cgic_Init(Tcl_Interp *interp, int argc, char **argv) {
#endif
	int result;
	char *cgiContentLengthString;

	Tcl_CreateCommand(interp, "cgi", Cgic_Commands, NULL, NULL);
	/*printf("Created cgi command\n");
	fflush(stdout);*/

	cgiSetupConstants();
  cgiGetenv(&cgiServerSoftware,      "SERVER_SOFTWARE");  Tcl_SetVar(interp, "SERVER_SOFTWARE",   cgiServerSoftware,      TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiServerName,          "SERVER_NAME");      Tcl_SetVar(interp, "SERVER_NAME",       cgiServerName,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiGatewayInterface,    "GATEWAY_INTERFACE");Tcl_SetVar(interp, "GATEWAY_INTERFACE", cgiGatewayInterface,    TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiServerProtocol,      "SERVER_PROTOCOL");  Tcl_SetVar(interp, "SERVER_PROTOCOL",   cgiServerProtocol,      TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiServerPort,          "SERVER_PORT");      Tcl_SetVar(interp, "SERVER_PORT",       cgiServerPort,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiRequestMethod,       "REQUEST_METHOD");   Tcl_SetVar(interp, "REQUEST_METHOD",    cgiRequestMethod,       TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiPathInfo,            "PATH_INFO");        Tcl_SetVar(interp, "PATH_INFO",         cgiPathInfo,            TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiPathTranslated,      "PATH_TRANSLATED");  Tcl_SetVar(interp, "PATH_TRANSLATED",   cgiPathTranslated,      TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiScriptName,          "SCRIPT_NAME");      Tcl_SetVar(interp, "SCRIPT_NAME",       cgiScriptName,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiQueryString,         "QUERY_STRING");     Tcl_SetVar(interp, "QUERY_STRING",      cgiQueryString,         TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiRemoteHost,          "REMOTE_HOST");      Tcl_SetVar(interp, "REMOTE_HOST",       cgiRemoteHost,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiRemoteAddr,          "REMOTE_ADDR");      Tcl_SetVar(interp, "REMOTE_ADDR",       cgiRemoteAddr,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiAuthType,            "AUTH_TYPE");        Tcl_SetVar(interp, "AUTH_TYPE",         cgiAuthType,            TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiRemoteUser,          "REMOTE_USER");      Tcl_SetVar(interp, "REMOTE_USER",       cgiRemoteUser,          TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiRemoteIdent,         "REMOTE_IDENT");     Tcl_SetVar(interp, "REMOTE_IDENT",      cgiRemoteIdent,         TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiContentType,         "CONTENT_TYPE");     Tcl_SetVar(interp, "CONTENT_TYPE",      cgiContentType,         TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiContentLengthString, "CONTENT_LENGTH");   Tcl_SetVar(interp, "CONTENT_LENGTH",    cgiContentLengthString, TCL_GLOBAL_ONLY);
  cgiContentLength = atoi(cgiContentLengthString);
  cgiGetenv(&cgiAccept,              "HTTP_ACCEPT");      Tcl_SetVar(interp, "HTTP_ACCEPT",       cgiAccept,              TCL_GLOBAL_ONLY);
  cgiGetenv(&cgiUserAgent,           "HTTP_USER_AGENT");  Tcl_SetVar(interp, "HTTP_USER_AGENT",   cgiUserAgent,           TCL_GLOBAL_ONLY);
#ifdef CGICDEBUG
	CGICDEBUGSTART
	fprintf(dout, "%d\n", cgiContentLength);
	fprintf(dout, "%s\n", cgiRequestMethod);
